...
class Decomposition:
    ...
    def __init__(self, peak_locs, peak_widths, peak_heights, k, Abs):
        #INITIALIZE k AND ABS? Convert to Numpy Arrays outside of class
    ...
    #Might be able to just stick this into initialization
    def theta_ravel(self):
        return np.vstack((self.peak_locs,self.peak_widths,self.peak_heights )).T.ravel()

    def theta(self):
        return self.theta0_ravel.reshape((-1,3))
    ...
    def construction(self, theta): #add theta as param for residuals function
        construction = np.sum(self.plot_gauss(self.theta),axis=0)
        return construction

    def baseline_subtract(self,x,amide_data,k,Abs):
        baseline = (Amide_I_Region.baseline_param(k,Abs)[0] * x) + Amide_I_Region.baseline_param(k,Abs)[0]
        return amide_data - baseline

    def residuals(self,theta_ravel):
        theta_ravel = self.theta_ravel()
        #first we have to get from size of data array to theoretical/guess array
        guess_data = np.interp(self.k,np.linspace(1600,1700,300),self.construction)
        #need to perform baseline subtraction, but how do we reference k and Abs out of scope of function?
        return self.baseline_subtract(self.k, self.Abs, self.k, self.Abs) - guess_data

    def decompose(self):
        fit_params = least_squares(self.residuals, self.theta_ravel, method='lm').x
        return fit_params
...
Decomposition = Decomposition(peaksA,widthsA,heights,df.k.to_numpy(),df.Abs.to_numpy())
#Now that an instance of decomposition class has been initialized, test all of the class functions
"""
